<template>
<div class="jyc-xjd">
  <step-shower :step="step" v-if="product !== 'XYG'"></step-shower>
    ...
    <div class="instert-bank-form">
      <ul>
        <template v-if="bankInfo">
          <li>
            <label>姓名</label>
            <p class="txt">{{bankInfo.bankPerson || bankInfo.custName}}</p>
          </li>
          <li>
            <label>身份证</label>
            <p class="text">{{bankInfo.ident || ident}}</p>
          </li>
          <li>
            <label>银行卡号</label>
            <p class="tel">{{bankInfo.bankNo || bankInfo.bankCode}}</p>
          </li>
          <li>
            <label>所属银行</label>
        ...
        <li>
          <label>手机号码</label>
          <input type="text" class="inp" v-model="phone" maxlength="11" disabled placeholder="请输入银行预留手机号">
        </li>
        <li>
          <label>验证码</label>
          <input type="tel" class="inp" v-model="vCode" maxlength="6" placeholder="请输入验证码">
          <span class="instert-bank-checekcode" v-if="!sendStatus" @click="getVcode">{{vMessage}}</span>
          <span class="instert-bank-checekcode" v-else><label v-if="timer > -1">{{timer}}s</label></span>
        </li>
      </ul>
    </div>
    <!-- 收不到验证码提示 -->
  ...
  <div class="footer">
    <p class="bnt1" @click="submitBind" v-preventReClick>提交</p>
...
export default {
  components: {
    stepShower, InsertBank, codeTips
  },
  data(){
    return{
      termStatic:true,
  ...
  async mounted(){
    ...
    // } else {
    //   this.phone = this.bankInfo && this.bankInfo.bankPhone
    // }
    this.getDetail()
  },
  methods:{
    ...
    async getDetail(){
      ...
      if(res.code === 'sc0' && res.data){
        ...
        this.set_bankInfo(info);
        this.phone = info.bankMobile;
      }
      else{
        this.$toast("请求失败：返回数据" + JSON.stringify(res));
      }
    },
    // 校验手机号
    checkPhone(ev) {
      ...
      if (this.phone && this.phone.length == 11) {
        if (this.checkPhone()) {
          let params = {
            ...
            bankNo: this.bankInfo.bankNo || this.bankInfo.bankCode,   //String银行卡号
            bankName: this.bankInfo.bankName,//String银行名称
            name: this.bankInfo.bankPerson || this.bankInfo.custName, //String姓名
          ...
          const res = await this.$api.bindBankSendCodeHttp(params);
    ...
    async submitBind(){
      ...
      let params = {
        ...
        bankNo: this.bankInfo.bankNo|| this.bankInfo.bankCode,//String银行卡号
        funding: this.$route.query.funding,//String资金方 BS
        bankPhone: this.bankInfo.bankPhone || this.phone,//String银行预留手机
        bankName: this.bankInfo.bankName,//String银行名称
      ...
      const res = await this.$api.bindBankSubmitHttp(params);
      if (res.code === '0' || res.code === 'sc0') {
        if(this.product === 'XYG'){
          if(this.$route.query.isSingleFunding || this.$route.query.isLastBind){
            this.goLink({
          ...
          if(this.$route.query.status == 1000){
              window.location.href = window.location.origin + '/custom-page/status-pages/#/jxh-contractList?orderNo='+ this.$route.query.orderNo + '&origin=fromApp';
          }
        }else{
          this.goLink({
            path: '/patch',
