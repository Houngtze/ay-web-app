<template>
  <div class="base-info">
    <div class="base-info-form">
      ...
      <span class="warn">{{defaultWords.underTxt}}</span>
    </div>
    <div class="base-info-btn">
      <button @click="submitForm">{{defaultWords.bntTxt}}</button>
    </div>
    <picker-pop
      :slotArr="bankList"
...
import PickerPop from "@/components/credit/components/PickerPop";
...
export default {
  ...
  data() {
    return {
      ...
      // protocol: [],
      defaultWords:{
        underTxt:'录入身份信息仅作为认证使用，不作其他用途，请放心输入',
        bntTxt:'提交'
      }
    };
  },
  ...
  created() {
    // this.getAgreement();
    Object.assign(this.defaultWords, this.words);
    this.getBankList();
  },
  methods: {
...
.base-info {
    ...
    ul {
      li {
        i {
          ...
          &.form-icon-code {
            @include bg-image("./images/icon-camera");
          }
          &.form-icon-bank {
            @include bg-image("./images/icon-bank");
          }
        }
  ...
  .base-info-btn {
    ...
    button {
      @extend %btn;
      height: 0.92rem;
      @include bg-image("./images/bg-btn");
      &:active {
        @include bg-image("./images/bg-btn-active");
      }
    }
    p{
      display: flex;
      ...
      .checkbox{
        ...
        flex-shrink: 0;
        @include bg-image("./images/icon-check");
        background-size: 100%;
        &.checked {
