<template>
  <div class="add-address">
    <div class="container" v-if="address && address.length>0">
      <div class="addr-li" v-for="(item,index) in address" :key="index">
        <div class="addr-msg" @click="selectAddr(item)">
          <p class="name-txt">收货人：{{item.userName}}<span class="tel">{{item.phone}}</span></p>
          <p class="addr-txt" v-if="item.alipayAccount">支付宝账号：{{item.alipayAccount}}</p>
          <p class="addr-txt" v-if="item.wechat">微信账号：{{item.wechat}}</p>
        </div>
        <div class="addr-bnt">
          <label @click="setDefaultAddr(item.id)">
            <input class="default-bnt" :value="item.id" v-model="defaultRadio" name="default-bnt" type="radio">
            <i class="icon"></i>默认收款账户
          </label>
    ...
    <div class="footer">
      <p class="cont-bnt" @click="goToAddAddr"><img src="./../../images/add-icon.png">新增支付宝或微信</p>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async setDefaultAddr(newRadio){
        ...
        let esp = await this.$api.socialAccountHttp(params);
        if(esp.code === 'sc0'){
          this.toast('设置成功');
        }
        else{
