<template>
  <div class="page-layout">
    <top-notice text="一对一客户经理服务 最高30万额度" />
    <!-- 数据面板 -->
    <main-panel :data="loanData" @onActiveChange="handleChange"  v-if="pageShow"/>
    <!-- 选择器 -->
    <option-selector :selectIndex="currentSelectorIndex" :data="data" @onActiveSelect="handleSelect" v-show="!isShowSubmit"  v-if="pageShow"/>
    <!-- 提交 -->
    <div v-show="isShowSubmit" class="submit" v-if="pageShow">
      <template v-if="!submitSuccess">
        <button @click="submitLoan">提交</button>
        <div class="term">
          <label><input type="checkbox" v-model="termStatic"><i class="icon"></i>点击即代表您已同意</label>
          <a href="http://aiyong.szqk-tech.com/terms/user-auth-agreement.html">《用户授权协议》</a>
        </div>
      </template>
       <template v-else>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (session.get("dajin-loan-select-data")) {
        this.loanData = session.get("dajin-loan-select-data")
    ...
    methods: {
      ...
      handleSelect(data) {
        ...
        session.set("dajin-loan-select-data", this.loanData)
        if (this.loanData.length !== this.data.length) {
