  ...
  <div class="txzf-home">
    <ul class="list">
      <li v-for="(item, index) in bankListData" :key="index" @click="setBankInfo(item)">
        <img class="bg-img" :src="item.backgroundUrl || item.backgroundNewUrl">
        <div class="msg">
    ...
    <div class="bnt">
      <p @click="goLink('/add-bankcard')">添加</p>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getBankList() {
        ...
        let res
        if (env.isJXHAPP()) {
          res = await this.$api.JXHgetBank(params);
        } else {
          res = await this.$api.getBankInfo(params);
        }
