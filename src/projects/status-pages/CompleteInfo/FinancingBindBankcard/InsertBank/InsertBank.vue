  ...
  <div class="instert-bank">
    <div class="instert-bank-form">
      <ul>
        ...
        <li>
          <label>身份证号</label>
          <input type="text" maxlength="21" v-model="ident" @blur="checkBankCode" placeholder="请填写您的身份证号">
        </li>
        <li>
          <label>手机号码</label>
...
import {imgCompression} from '../../../utils/index'
export default {
  components: {
    ProtocolItem,
    PickerPop
  ...
  created() {
    ...
    if (/\bAiYongApp\b/.test(ua)) {

    }
    console.log(env.isApp())
    this.getBankList();
  },
  methods: {
    // 获取银行列表
    ...
    async getVcode(val) {
      if (this.sendStatus)  return false
      if (this.phone && this.phone.length == 11) {
        if (this.checkPhone() && this.bankCode) {
          let res
          if (env.isApp()) {
            res = await this.$api.SendSmsCode({
              codeType: "addBankCard",
              phone: this.phone
            })
          } else {
            res = await this.$api.JXHsendSmsCode({
    ...
    async fileVal(ev) {
      ...
      const res = await this.$api.fileUpload(formData);
      if (res.code === 'sc0'){
    ...
    async submitForm() {
      ...
      let res
      if (env.isApp()) {
        res = await this.$api.addCreditBankCard(param);
      } else {
        res = await this.$api.JXHaddBank(param);
      }
      if (res.code === 'sc0' || res.code === '0') {
        // 退回上一页
        this.$router.go(-1)
